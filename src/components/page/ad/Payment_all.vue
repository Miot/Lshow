    ...
    <div id="container">
        <div class="timePicker">
            <div class="side" @click="preDate()">
                <img src="../../../../static/img/arrow_left_g.png">
                前一天
            </div>
            <span>{{showDate}}</span>
            <div class="side" @click="nextDate()">
                后一天
                <img src="../../../../static/img/arrow_right_g.png">
            </div>
        </div>
            ...
            <v-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" :auto-fill="false" class="dailyList">
                <ul>
                    <li v-for="(item,index) in mainList" :key="index" @click="goDetail(item.id)">
                        <!-- 左侧图标 -->
                        <div class="left icon">
                            <img src="../../../../static/img/u3883.png">
                            <div class="trigon right"></div> 
                        </div>
                        <!-- 中间数据信息 -->
                        <div class="data">
                            <div class="left" style="flex:3">
                                <p style="font-size:0.3rem">{{item.outlayText}}</p>
                                <p>{{item.outlayTime}}</p>
                            </div>
...
export default {
    ...
    methods:{
        ...
        getList(opt){
            ...
            this.opt = opt;
            Indicator.open();
            this.LKshow.api.account.getOutLogs({
                data:{
                    size:_this.pageSize,
                    index:_this.pageIndex,
                    opt:opt,
                    outlayDate:_this.currentDate
                },
...
.ed_footer{
    ...
    .active{
        background:linear-gradient(225deg, rgba(0, 153, 0, 1) 0%, rgba(0, 153, 0, 1) 0%, rgba(51, 255, 51, 1) 100%, rgba(51, 255, 51, 1) 100%);
        color:#fff;
    }
}
</style>
